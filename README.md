...
```mermaid
graph TD
    User[ユーザー] -->|指定時刻| Schedule[スケジューラー]
    Schedule -->|トリガー| Weather[天気予報 API]
    
    subgraph Raspberry Pi 4
        Weather -->|降水確率判定| Logic{雨予報？}
        Logic -- Yes --> Camera[USBカメラ]
        Camera -->|画像取得| Vision[画像認識 (YOLO-Tiny)]
        Vision -->|傘の有無| Judge{傘はある？}
        
        Judge -- No (危険) --> Voice1[警告メッセージ]
        Judge -- Yes (安心) --> Voice2[称賛メッセージ]
        Logic -- No (晴れ) --> Voice3[天気予報のみ]
        
        Voice1 & Voice2 & Voice3 --> Speaker[スピーカー]
    end
```
